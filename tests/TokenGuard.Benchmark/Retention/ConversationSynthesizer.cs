...
/// </exception>
    public SyntheticConversation Synthesize(ScenarioProfile profile)
    {
        ...
        for (var turnIndex = 0; turnIndex < profile.TurnCount; turnIndex++)
        {
            ...
            var userMessage = this.BuildMessage(MessageRole.User, userSegments, templates, random, targetTokensPerMessage);
            var modelMessage = this.BuildMessage(MessageRole.Model, modelSegments, templates, random, targetTokensPerMessage);
