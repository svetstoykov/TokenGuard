...
TokenGuard fills that gap.

---

## What TokenGuard Does

TokenGuard sits inside your agent loop and manages the conversation context automatically. It monitors token usage, decides when compaction is needed, and applies the appropriate strategy — all behind a clean interface that requires minimal changes to your existing code.

You keep writing your agent loop. TokenGuard keeps the context healthy.
...
1. **Drop-in, not rip-and-replace.** TokenGuard should be adoptable in an afternoon. It wraps your existing message list — it doesn't replace your LLM client, your tool execution, or your agent orchestration.
...
3. **Sensible defaults, full control.** Out of the box, TokenGuard should work well without tuning. But every threshold, window size, and strategy parameter should be overridable for teams that need precision.

4. **Provider-agnostic.** TokenGuard works with any LLM provider. It operates on a standard message abstraction — it doesn't care whether the underlying model is Claude, GPT, Gemini, or a local model.

---

## Context Management Strategies

Based on current research (JetBrains Research, "Cutting Through the Noise," NeurIPS 2025; Claude Code's three-tier compaction system; OpenHands; SWE-agent), TokenGuard is currently centered on a sliding-window masking implementation and is planned to support three core strategies, plus a hybrid approach, as the library expands.
...
**How it works:**
- When context exceeds a threshold, TokenGuard sends the conversation to an LLM with a summarization prompt.
...
TokenGuard needs a standard representation of a conversation turn that is provider-agnostic. This abstraction should capture:
...
A `ContextBudget` defines the constraints TokenGuard operates within:
...
When TokenGuard compacts, it produces a `CompactionEvent` that includes:
...
### Where TokenGuard Lives in the Loop
...
TokenGuard currently ships with:
...
- **Raw/Custom** — a generic adapter for any provider, using TokenGuard's own message type.

### Token Counting

Accurate token counting is critical for compaction decisions. TokenGuard supports:
...
The summary is wrapped in a structured format (XML or JSON) so TokenGuard can parse, store, and re-inject it reliably.
...
After compaction, TokenGuard supports an optional rehydration step inspired by Claude Code's post-compaction reconstruction. This is particularly valuable for the Tiered and Hybrid strategies:
...
- **Resource re-injection** — if the agent was working with specific files or data, TokenGuard can re-read and re-inject the most recent resources (configurable, capped at a token budget).
...
| **TokenGuard** | **.NET** | **Multiple strategies, provider-agnostic** | **New, unproven** |

The .NET ecosystem has no dedicated, provider-agnostic context management library. TokenGuard is the first.
...
- **Adoptable**: A developer can integrate TokenGuard into an existing agent loop in under 30 minutes.
